import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@xxch-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@xxch-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@xxch-network/api-react';
import { catToMojo, xxchToMojo } from '@xxch-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXXCH = useWatch({
    name: 'requested.xxch',
  });

  const offeredXXCH = useWatch({
    name: 'offered.xxch',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXXCH ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('xxch', xxchToMojo(amount))),
    ...(requestedTokens ?? [])
