...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (XXCH), indicates whether a token or CAT has a default entry
 ...
 * for creating an XXCH offer, a CAT offer, or an NFT offer. XXCH and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      xxch: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
