// import { useGetNFTInfoQuery } from '@xxch-network/api-react';
import { fromBech32m } from '@xxch-network/api';
import { AddressBookContext, Color, EmojiAndColorPicker, Flex, Form, TextField } from '@xxch-network/core';
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[4] === '1') {
          if (entry.address.slice(0, 4).toLowerCase() !== 'xxch') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[5] === '1') {
          if (entry.address.slice(0, 5).toLowerCase() !== 'txxch') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
