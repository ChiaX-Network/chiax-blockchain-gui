    ...
    ipcRenderer.invoke('fetchHtmlContent', latestVersionURL).then((obj: any) => {
      try {
        ...
        /* we don't need to handle error here, if we are unable to fetch version number
           from xxch.cc, we just ignore showing reminder dialog */
      }
    });
  }, [latestVersionURL]);

  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.xxch.cc/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.xxch.cc/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.xxch.cc/').toString() : undefined;
