...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToXxch, mojoToCAT } from '@xxch-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xxch', 'txxch'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToXxch(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xxch', 'txxch'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToXxch(takerEntries[0][1])
