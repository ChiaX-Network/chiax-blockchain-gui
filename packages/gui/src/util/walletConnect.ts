...
const log = initDebug('xxch-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `xxch_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.xxch;
    if (!requiredNamespace) {
      throw new Error('Missing required xxch namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['xxch:testnet', 'xxch:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `xxch:${instance}:${fingerprint}`);
