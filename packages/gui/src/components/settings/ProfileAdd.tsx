...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'XXCH').toUpperCase();
  const isTestnet = currencyCode === 'TXXCH';
  ...
  const spendableBalance = mojoToXxchLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet10-faucet.xxch.cc' : 'faucet.xxch.cc'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the Xxch Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
