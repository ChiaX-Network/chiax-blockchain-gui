...
import type { CATToken, Wallet } from '@xxch-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@xxch-network/api-react';
import { useCurrencyCode } from '@xxch-network/core';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const xxchWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'Xxch',
        symbol: currencyCode,
        displayName: `Xxch (${currencyCode})`,
        disabled: false,
        tail: '',
      })
    );
    ...
    const allOptions = [...xxchWalletSelection, ...catOptions];
