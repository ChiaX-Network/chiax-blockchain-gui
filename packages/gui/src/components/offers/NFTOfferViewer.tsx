import { OfferSummaryRecord, OfferTradeRecord } from '@xxch-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@xxch-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['xxch', 'txxch'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.XXCH: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const xxchMakerFee = mojoToXxch(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(xxchMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.XXCH
        ? xxchToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
