import { toBech32m, WalletType } from '@xxch-network/api';
import { mojoToXxch, mojoToCAT } from '@xxch-network/core';
...
export default function createOfferForIdsToOfferBuilderData(
      ...
      if (asset) {
        switch (asset.walletType) {
          case WalletType.STANDARD_WALLET:
            section.xxch.push({ amount: mojoToXxch(numericValue.abs()).toFixed() });
            break;
          case WalletType.CAT:
  ...
  if (fee) {
    offerBuilderData.offered.fee = [{ amount: mojoToXxch(fee).toFixed() }];
  }
