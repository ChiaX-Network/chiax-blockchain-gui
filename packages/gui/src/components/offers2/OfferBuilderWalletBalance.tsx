...
import { useWallet } from '@xxch-network/wallets';
...
export default function OfferBuilderWalletBalance(props: OfferBuilderWalletBalanceProps) {
  ...
  const xxchBalance = useMemo(() => {
    if (isLoading || !wallet || !walletBalance || !('spendableBalance' in walletBalance)) {
      return undefined;
    }

    if (wallet.type === WalletType.STANDARD_WALLET) {
      return mojoToXxchLocaleString(walletBalance.spendableBalance, locale);
    }
  ...
  if (!isLoading && xxchBalance === undefined) {
    return null;
  }
