...
const PY_DIST_FILE = 'xxch_daemon';
const PY_FOLDER = '../../../xxch/daemon';
...
const getXxchVersion = () => {
  let version = null;
  const exePath = getExecutablePath('xxch');
  // first see if we can get a xxch exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or xxch is on the path
    try {
      version = childProcess
