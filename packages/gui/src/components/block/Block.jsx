import { toBech32m } from '@xxch-network/api';
import { useGetBlockQuery, useGetBlockRecordQuery } from '@xxch-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToXxch(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToXxch(calculateBaseFarmerReward(blockRecord.height));

  const xxchFees = blockRecord.fees !== undefined ? mojoToXxch(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: xxchFees ? `${xxchFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the Xxch blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
