import { Flex, Loading, TooltipIcon } from '@xxch-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `xxch_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.xxch?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the Xxch application may not support some commands requested by the
