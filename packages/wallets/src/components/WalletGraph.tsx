import { TransactionType, WalletType } from '@xxch-network/api';
import type { Transaction } from '@xxch-network/api';
import { useGetWalletBalanceQuery } from '@xxch-network/api-react';
import { Color, mojoToXxch, mojoToCAT, blockHeightToTimestamp } from '@xxch-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(
        0,
        ([WalletType.CAT, WalletType.CRCAT].includes(walletType) ? mojoToCAT(start) : mojoToXxch(start)).toNumber()
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip: [WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(start)
        : mojoToXxch(start).toString(), // bignumber is not supported by react
    });
  });
