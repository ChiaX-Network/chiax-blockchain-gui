import { Button, Flex, useOpenDialog, xxchToMojo } from '@xxch-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeXxch = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeXxch) {
        const feeMojos = feeXxch ? xxchToMojo(feeXxch).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
