import { OfferTradeRecord } from '@xxch-network/api';
import {
  ButtonLoading,
  CopyToClipboard,
  DialogActions,
...
const log = debug('xxch-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.xxch.cc';
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50_029, // Pairing doesn't exist e.g. XXCH/RandoCoin
...
async function postToHashgreen(offerData: string, testnet: boolean): Promise<string> {
  ...
  if (statusCode === 200) {
    ...
    if (testnet) {
      return 'https://www.xxch.cc/offers';
    }
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<{ viewLink: string; offerLink: string }> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'txxch' : 'xxch'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
