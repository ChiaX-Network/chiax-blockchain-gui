import { ConfirmDialog, Flex } from '@xxch-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'XXCH' && assetStatus.type === 'XXCH') {
      // assetId might be either 'XXCH' or 'UNKNOWN'(maybe fee) but here both are treated as just a XXCH spending
