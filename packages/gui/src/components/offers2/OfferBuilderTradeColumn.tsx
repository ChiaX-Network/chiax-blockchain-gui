import { Color, Flex } from '@xxch-network/core';
import { Offering, Requesting } from '@xxch-network/icons';
...
import OfferBuilderXXCHSection from './OfferBuilderXXCHSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showXXCH = !readOnly || !!xxch.length;
  ...
  const mutedXXCH = nfts.length || tokens.length;
  const mutedTokens = xxch.length || nfts.length;
  const mutedNFTs = xxch.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showXXCH && <OfferBuilderXXCHSection name={`${name}.xxch`} offering={offering} muted={mutedXXCH} />}
