import { useGetFeeEstimateQuery } from '@xxch-network/api-react';
...
import mojoToXxchLocaleString from '../../utils/mojoToXxchLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendXXCH = 'send_xxch_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToXxchLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const xxchFee = mojoToXxchLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, xxchFee);
        }
      }
    }
