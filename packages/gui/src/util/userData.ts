...
export function getUserDataDir(): string {
  const xxchRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(xxchRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const xxchRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${xxchRootUserDataPath}`);
  app.setPath('userData', xxchRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(xxchRootUserDataPath, 'Local Storage', 'leveldb');
