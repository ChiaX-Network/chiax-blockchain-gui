...
Welcome to the Xxch GUI repo!

Xxch GUI is written in TypeScript and uses Electron/React.

This monorepo consists of the following packages:

| Package name  | Description                                                                                                |
| ------------- | ---------------------------------------------------------------------------------------------------------- |
| **api**       | JS/TS library to access the Xxch Blockchain RPC                                                            |
...
| **icons**     | Xxch specific icons                                                                                        |
| **wallets**   | Common React components and hooks. Do not use this in you project. Will be merged to **core** package soon |

## Development

1. This repo (xxch-blockchain-gui) must be under xxch-blockchain repo. Please follow the [installation steps for the xxch-blockchain](https://github.com/Chia-Network/chia-blockchain/wiki/INSTALL#install-from-source). Make sure to install from source code (git clone...).
2. Run the `sh install-gui.sh` as instructed in the previous step. This will clone the xxch-blockchain-gui under xxch-blockchain repo.
3. Change directory into the xxch-blockchain-gui subdirectory.
4. Run `npm run dev:gui`

**When developing, please:**

- Only edit the code with the **Vscode editor**.
- Always have **xxch-blockchain-gui opened as a root folder in the Vscode** and not xxch-blockchain, or xxch-blockchain-gui/packages/... Failing to do so will result in incorrect auto linting and auto formatting which would not go through the CI quality checks.
...
- To install `lodash` for **single** package: `npx lerna add lodash --scope=@xxch-network/icons`
...
- For Xxch employees: **prefix your branch with your name**, like this: `yourName/what-is-the-code-about`. This helps when cleaning up old branches.
...
Do not edit files directly in the repo, but instead please head over to our [Crowdin project](https://xxch.crowdin.com/xxch-blockchain-gui) and add/edit translations there.
...
- **`npm run dev:gui` fails to start the app without providing a reason**
  ...
  1. In your command line, please go to the `xxch-blockchain` directory (one level up)
  2. Run `. ./activate`
  3. Run `cd xxch-blockchain-gui`
  4. Run `npm run dev:gui` to start the app.
  5. If still does not work, please open you process manager and kill all Xxch / Python related processes.
...
1. In the Xxch Electron app, click `View -> Developer -> Developer tools`.
...
1. Please follow the [Install and configure the simulator](https://docs.xxch.cc/guides/simulator-user-guide/). Do this step only once.
2. In the xxch-blockchain directory, run this to setup the ENV variables. Use these instead of the ones mentioned in the above guide.

```
export XXCH_ROOT=~/.xxch/simulator/main
export XXCH_SIMULATOR_ROOT=~/.xxch/simulator
export XXCH_KEYS_ROOT=~/.xxch_keys_simulator
```

3. `. ./activate`
4. `xxch start simulator`
5. `cd xxch-blockchain-gui/packages/gui`
6. `npm run dev:skipLocales`
7. You should see your simulator wallets. You should not see your testNet / mainNet wallets.
8. Run `xxch dev sim farm` as many times you want to farm some coins.
