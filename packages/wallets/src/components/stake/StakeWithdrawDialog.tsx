...
export default function StakeWithdrawDialog(props: Props) {
  const { onClose, open, coinId, amountInMojo, isStakeFarm, address } = props;
  ...
  const autoWithdrawFee = autoWithdrawData?.txFee ? mojoToXxch(autoWithdrawData.txFee) : 0;
  ...
  async function handleSubmit(values: FormData) {
    ...
    if (!isNumeric(fee)) {
      ...
      throw new Error(t`Please enter withdraw fee >= 0.1`);
    }
    ...
    openDialog(
      <AlertDialog title="">
        {isStakeFarm ? <Trans>Stake Farm Withdrawal</Trans> : <Trans>Stake Lock Withdrawal</Trans>}
      </AlertDialog>
    );
  }
  ...
  return (
    <Dialog onClose={handleDialogClose} maxWidth="lg" aria-labelledby="confirmation-dialog-title" open={open}>
      <DialogTitle id="confirmation-dialog-title" sx={{ minWidth: '550px' }}>
        {isStakeFarm ? <Trans>Stake Farm Transaction</Trans> : <Trans>Stake Lock Transaction</Trans>}
        <IconButton
          aria-label="close"
          onClick={onClose}
          sx={{
            ...
            <Flex gap={2} flexDirection="column" sx={{ textAlign: 'center', alignItems: 'center' }}>
              <Box sx={{ mb: 3 }}>
                <Typography variant="h5">
                  <FormatLargeNumber value={mojoToXxch(amountInMojo)} />{' '}
                  <Box
                    component="span"
