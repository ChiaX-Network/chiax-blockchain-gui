...
import { xxchToMojo, catToMojo } from '@xxch-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const feeInMojos = firstFee ? xxchToMojo(firstFee.amount) : new BigNumber(0);
        ...
        if (assetId.toUpperCase() === 'XXCH' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have xxch spending which is only XXCH, only Fee or both XXCH and Fee
              pendingOffers[idx].assetId = 'XXCH+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'XXCH',
  ...
  const xxchTasks = offeredXch.map(async (xxch) => {
    ...
    const mojoAmount = xxchToMojo(amount);
  ...
  requestedXch.forEach((xxch) => {
    ...
    walletIdsAndAmounts[wallet.id] = xxchToMojo(amount);
  });
