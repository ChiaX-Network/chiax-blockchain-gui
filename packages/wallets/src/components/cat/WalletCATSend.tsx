import { SyncingStatus, toBech32m, WalletType } from '@xxch-network/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@xxch-network/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('xxch_addr') || address.includes('colour_desc')) {
